...
class MetadataService:
    """
    Builds metadata summaries from Sheet Mapping rows.

    Used by UI and diagnostics to show:
    - Record summaries
    - Column rows
    - Relationship summaries
    - Sheet Mapping preview rows
    """
    ...
    def _infer_parent_record_from_reference(
        ...
        if row.cross_application_db2_table:
            return NameNormalizer.normalize(row.cross_application_db2_table)
