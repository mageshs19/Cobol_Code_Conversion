...
def render_main_tab() -> None:
    ...
    with col1:
        ...
        dclgen_files = st.file_uploader(
            ...
            help=(
                ...
                "because production DCLGEN files may not use .txt or .cpy."
            ),
        )
    ...
    target_program_id = st.text_input(
        ...
        help=(
            ...
            "and becomes the default output .cbl file name."
        ),
    )
    ...
    auto_fix_pic_length_mismatches = st.checkbox(
        ...
        help=(
            "Reserved for later transformer batch. Current clean skeleton "
            "stores the setting but does not yet apply PIC expansion."
        ),
    )
...
def load_inputs(
    ...
    if sheet_mapping_file is None:
        ...
        diagnostics.append(
            f"Sheet Mapping size bytes: {len(sheet_mapping_file.getvalue())}"
        )

        try:
            sheet_rows = sheet_parser.parse_uploaded_file(sheet_mapping_file)
    ...
    if idms_cobol_source_file is None:
        ...
        try:
            ...
            diagnostics.append(
                f"IDMS COBOL source uploaded: {idms_cobol_source_name}"
            )
            diagnostics.append(
                f"IDMS COBOL source text length: {len(idms_cobol_text)}"
            )
        except Exception as exc:
    ...
    if dclgen_files and not dclgen_columns:
        st.warning(
            "DCLGEN files were uploaded but parsed as 0 columns. "
            "Open the Diagnostics tab to review detected table and field parsing."
        )

    if idms_cobol_source_file is not None and not idms_cobol_text.strip():
        st.warning(
            "IDMS COBOL source file was uploaded but no text was read. "
...
def render_metadata_overview_tab() -> None:
    ...
    with col1:
        st.metric("Total Records", len(record_summaries))

    with col2:
        st.metric("Total Sheet Mapping Rows", len(st.session_state.sheet_mapping_rows))

    with col3:
        st.metric("Total DCLGEN Columns", len(st.session_state.dclgen_columns))
...
def render_column_names_tab() -> None:
    ...
    metadata_service = MetadataService()
    rows = metadata_service.column_rows(st.session_state.sheet_mapping_rows)
